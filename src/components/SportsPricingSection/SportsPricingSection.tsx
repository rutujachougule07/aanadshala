...
const SportsPricingSection = () => {
  ...
  const sportsRateItems =
    store.sportsPricingItems && store.sportsPricingItems.length > 0
      ? store.sportsPricingItems
  ...
  const pkgFreeTrialPass: PackageDetail = {
    ...
    savings: isEn
      ? "100% Free Pass • Zero Fee, Zero Conditions!"
      : "१००% मोफत पास • शून्य शुल्क, शून्य अट!",
    facilityNote: isEn
  ...
  return (
    <>
      <section className="ps-exact-table-section" id="sports-pricing">
        ...
        <div className="ps-table-wrapper max-w-5xl mx-auto">
          <table className="ps-exact-table w-full">
            <thead>
              <tr>
                <th className="th-details text-left py-4 px-5">
                  {isEn ? "Membership Duration" : "कालावधी (MEMBERSHIP DURATION)"}
                </th>
                <th className="py-4 px-4 text-center">
                  {isEn ? "Rack Rate (₹)" : "मूळ दर (RACK RATE ₹)"}
                </th>
                <th className="py-4 px-4 text-center">
                  {isEn ? "Pre-Launch Offer (₹)" : "प्री-लाँच दर (OFFER ₹)"}
                </th>
                <th className="py-4 px-4 text-center">
                  {isEn ? "Savings (₹)" : "एकूण बचत (SAVINGS ₹)"}
                </th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={tier.id || idx} className="ps-table-row">
                  <td className="td-title-cell text-left py-4 px-5 font-black text-slate-900 text-base">
                    <div className="flex items-center gap-3">
                      <span className="row-num">{formatNum((idx + 1).toString())}</span>
                      <div>
                        <span className="row-cat-label">
                          {isEn ? tier.badgeEn || tier.badgeMr : tier.badgeMr}
                        </span>
                        <div className="row-title-text font-black text-slate-900 text-base">
                          {isEn ? tier.durationEn || tier.durationMr : tier.durationMr}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="price-bold text-slate-900 text-center font-black text-base">
                    ₹ {formatNum(tier.rackRate)}
                  </td>
                  <td className="price-bold text-center font-black text-[#db2777] text-lg sm:text-xl">
                    ₹ {formatNum(tier.offerPrice)}
                  </td>
                  <td className="price-bold text-center font-black text-[#059669] bg-[#ebfbf3] text-base">
                    ₹ {formatNum(tier.savings)} {isEn ? "Savings" : "बचत"}
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          {(
            ...
            <div key={tier.id || idx} className="ps-mobile-item-card">
              <div className="mobile-card-header">
                ...
                <span className="mobile-cat-tag">
                  {isEn ? tier.badgeEn || tier.badgeMr : tier.badgeMr}
                </span>
              </div>
              <div className="mobile-title-text font-black">
                {isEn ? tier.durationEn || tier.durationMr : tier.durationMr}
              </div>
              <div className="mobile-prices-grid grid-cols-3">
                <div className="mobile-price-box">
                  ...
                  <span className="mobile-price-val text-slate-900 font-black">
                    ₹ {formatNum(tier.rackRate)}
                  </span>
                </div>
                <div className="mobile-price-box">
                  <span className="mobile-price-lbl">{isEn ? "Offer Rate" : "ऑफर दर"}</span>
                  <span className="mobile-price-val text-[#db2777]">
                    ₹ {formatNum(tier.offerPrice)}
                  </span>
                </div>
                <div className="mobile-price-box">
                  <span className="mobile-price-lbl">{isEn ? "Savings" : "बचत"}</span>
                  <span className="mobile-price-val text-[#059669]">
                    ₹ {formatNum(tier.savings)}
                  </span>
                </div>
              </div>
            </div>
      ...
      {selectedDetail &&
        createPortal(
          <div className="sp-modal-overlay" onClick={() => setSelectedDetail(null)}>
            <div
            ...
            >
              ...
              <div className="bg-linear-to-r from-pink-50 via-purple-50 to-indigo-50 p-4 rounded-2xl border border-pink-200 mb-5">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-xs text-slate-500 font-bold">
                      {isEn ? "Regular Price: " : "मूळ दर (Rack Rate): "}
                      <span className="line-through text-slate-400">{selectedDetail.rackRate}</span>
                    </div>
                    <div className="text-2xl font-black text-pink-600">
                      {selectedDetail.offerPrice}{" "}
              ...
              <ul className="space-y-2.5 mb-6">
                {selectedDetail.benefits.map((benefit, i) => (
                  <li
                    key={i}
                    className="flex items-start gap-2.5 text-slate-800 text-xs font-bold bg-white p-3 rounded-xl border border-pink-100 shadow-xs"
                  >
                    <span className="size-5 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">
                      ✓
                    </span>
                    <span>{benefit}</span>
                  </li>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <button
                  ...
                  className="flex-1 py-3.5 rounded-full bg-linear-to-r from-pink-600 to-purple-600 hover:from-pink-700 hover:to-purple-700 text-white font-black text-xs sm:text-sm shadow-lg transition cursor-pointer flex items-center justify-center gap-2"
                >
              ...
              {!isSubmitted ? (
                <>
                  ...
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="sp-form-group">
                      ...
                      >
                        <option
                          value="१-दिवसाचा फ्री ट्रायअल पास (Free 1-Day Trial Pass)"
                          className="bg-[#0f172a] text-white font-bold"
                        >
                          १-दिवसाचा फ्री ट्रायअल पास (मोफत)
                        </option>
                        <option
                          value="1 Day Pass (₹300)"
                          className="bg-[#0f172a] text-white font-bold"
                        >
                          १ दिवस डे-पास (₹३००)
                        </option>
                        ...
                        <option
                          value="Annual Special Offer"
                          className="bg-[#0f172a] text-white font-bold"
                        >
                          विशेष वार्षिक ऑफर (₹१२,००० + GST)
                        </option>
                        <option
                          value="Preetam Elite Lifetime Membership"
                          className="bg-[#0f172a] text-white font-bold"
                        >
                          प्रीतम एलिट १० वर्षे फॅमिली मेंबरशिप
                        </option>
                      </select>
                    </div>
                ...
                <div className="text-center py-6">
                  ...
                  <p className="text-sm font-bold text-slate-300 mb-6">
                    धन्यवाद <strong>{formData.name}</strong>! तुमची मेंबरशिप चौकशी सबमिट झाली आहे.
                    आमची टीम लवकरच फोन करेल.
                  </p>
