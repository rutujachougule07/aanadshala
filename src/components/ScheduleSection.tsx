      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-10">
          ...
          <h2 className="text-2xl sm:text-4xl md:text-5xl font-black tracking-tight drop-shadow-md flex items-center justify-center flex-wrap gap-2 md:gap-3 py-1.5 bg-linear-to-r from-[#1A05A2] via-purple-700 to-[#db2777] bg-clip-text text-transparent">
            ...
            <span>
              {isEn
                ? "Preetam Sports Club Timetable"
                : config.headerTitle || "प्रीतम स्पोर्ट्स क्लब वेळापत्रक"}
            </span>
            <span className="text-amber-500 opacity-90 text-2xl sm:text-3xl">✨</span>
          </h2>
          ...
          {config.posterUrl && (
            <div className="mt-6">
              <button
                ...
                className="inline-flex items-center gap-3 px-6 py-3.5 rounded-2xl text-white font-extrabold text-sm sm:text-base shadow-xl hover:scale-105 transition-all cursor-pointer border-2 border-white/40 bg-linear-to-r from-[#1A05A2] to-purple-700 hover:shadow-indigo-500/40"
              >
                <span>📜</span>
                <span>
                  {isEn
                    ? "Official Timetable Uploaded by Admin (View / Download)"
                    : "ॲडमिनद्वारे अपलोड केलेले अधिकृत वेळापत्रक (पहा / डाउनलोड करा)"}
                </span>
                <span className="bg-amber-400 text-slate-900 text-xs px-2.5 py-0.5 rounded-full font-black">
                  {isEn ? "View" : "पहा"}
                </span>
              </button>
            </div>
            ...
            <div className="relative max-w-4xl w-full bg-white rounded-3xl p-4 sm:p-6 shadow-2xl space-y-4 my-auto">
              ...
              {config.posterType === "pdf" ? (
                <iframe
                  ...
                  className="w-full h-[70vh] rounded-2xl border"
                  title="Schedule PDF"
                />
              ) : (
                <div className="max-h-[75vh] overflow-y-auto rounded-2xl border border-slate-200">
                  <img
                    src={config.posterUrl}
                    alt="Official Timetable Schedule"
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between bg-white rounded-t-3xl border-b-4 border-[#1A05A2] shadow-sm p-4 px-6 md:px-12 z-10 relative mt-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[#f472b6] font-extrabold text-sm">
                {isEn ? "Schedule" : "वेळापत्रक"}
              </div>
              <div className="text-[#1a1a40] font-black text-base sm:text-lg">
                {isEn
                  ? "Monday to Sunday (Open All Days)"
                  : config.daysText || "सोमवार ते रविवार (सर्व दिवस खुली)"}
              </div>
            </div>
          </div>
        ...
        <div className="overflow-x-auto bg-white rounded-b-3xl shadow-xl border border-slate-100">
          <table className="w-full min-w-225 text-center border-collapse">
            <thead>
              <tr className="bg-linear-to-r from-[#f472b6] via-[#1A05A2] to-[#6a0dad] text-white">
                <th className="py-2.5 px-2 font-black text-base border-r border-white/20 w-[15%]">
                  {isEn ? "Time" : "वेळ"}
                </th>
                <th className="py-2.5 px-2 font-black text-base border-r border-white/20">
                  {isEn ? "Monday" : "सोमवार"}
                </th>
                <th className="py-2.5 px-2 font-black text-base border-r border-white/20">
                  {isEn ? "Tuesday" : "मंगळवार"}
                </th>
                <th className="py-2.5 px-2 font-black text-base border-r border-white/20">
                  {isEn ? "Wednesday" : "बुधवार"}
                </th>
                <th className="py-2.5 px-2 font-black text-base border-r border-white/20">
                  {isEn ? "Thursday" : "गुरुवार"}
                </th>
            ...
            <tbody>
              {items.map((row, i) => (
                <tr
                  key={row.id || i}
                  className={`border-b border-slate-200 hover:bg-pink-50/40 transition-colors ${i % 2 === 0 ? "bg-white" : "bg-[#fafafa]"}`}
                >
                  <td className="py-2.5 px-2 border-r border-slate-200">
                    <div className="flex flex-col items-center justify-center gap-0.5">
                      <span className="text-xl text-[#1A05A2]">{row.icon}</span>
                      <span className="font-extrabold text-[#1a1a40] text-xs sm:text-sm">
                        {row.time}
                      </span>
                    </div>
                  </td>
                  <td className="py-2 px-2 border-r border-slate-200">
                    <div className="font-extrabold text-[#1a1a40] text-sm sm:text-base leading-tight mb-0.5">
                      {row.mon?.main || ""}
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mt-8">
          ...
          <div className="col-span-1 lg:col-span-4 bg-white rounded-[24px] shadow-lg border-2 border-pink-200 overflow-hidden relative flex flex-col justify-between">
            <div className="bg-[#f472b6] text-white text-center py-2.5 font-black text-base flex justify-center items-center gap-2 w-max mx-auto px-8 rounded-b-xl absolute top-0 inset-x-0 shadow-sm z-10">
              <span className="text-xs">✦</span> {isEn ? "Our Key Features" : "आमच्या वैशिष्ट्ये"}{" "}
            ...
            <div className="pt-14 pb-5 px-4 grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-3 gap-2.5 sm:gap-3.5 items-center justify-center h-full">
              <div className="flex flex-col items-center text-center gap-1.5 p-2 rounded-xl bg-pink-50/60 hover:bg-pink-100/60 transition-colors border border-pink-100">
                <div className="w-11 h-11 rounded-full bg-[#1A05A2] text-white flex items-center justify-center text-xl shadow-md">
                  🏢
                </div>
                <div className="text-xs sm:text-sm font-black text-[#1a1a40] leading-tight">
                  {isEn ? "Modern Facilities" : "आधुनिक सुविधा"}
                </div>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 rounded-xl bg-blue-50/60 hover:bg-blue-100/60 transition-colors border border-blue-100">
                <div className="w-11 h-11 rounded-full bg-[#208dd7] text-white flex items-center justify-center text-xl shadow-md">
                  🛡️
                </div>
                <div className="text-xs sm:text-sm font-black text-[#1a1a40] leading-tight">
                  {isEn ? "Safety First" : "सुरक्षा प्रथम"}
                </div>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 rounded-xl bg-rose-50/60 hover:bg-rose-100/60 transition-colors border border-rose-100">
                <div className="w-11 h-11 rounded-full bg-[#f472b6] text-white flex items-center justify-center text-xl shadow-md">
                  ❤️
                </div>
                <div className="text-xs sm:text-sm font-black text-[#1a1a40] leading-tight">
                  {isEn ? "Health Care" : "आरोग्याची काळजी"}
                </div>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 rounded-xl bg-purple-50/60 hover:bg-purple-100/60 transition-colors border border-purple-100">
                ...
                <div className="text-xs sm:text-sm font-black text-[#1a1a40] leading-tight">
                  {isEn ? "Arts & Culture" : "मनोरंजन व संस्कृती"}
                </div>
              </div>
              <div className="flex flex-col items-center text-center gap-1.5 p-2 rounded-xl bg-pink-50/60 hover:bg-pink-100/60 transition-colors border border-pink-100 col-span-2 sm:col-span-1 lg:col-span-2">
                ...
                <div className="text-xs sm:text-sm font-black text-[#1a1a40] leading-tight">
                  {isEn ? "Expert Trainers" : "अनुभवी मार्गदर्शक"}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-1 lg:col-span-5 bg-white rounded-[24px] shadow-lg border-2 border-indigo-200 relative pt-16 pb-6 px-5 sm:px-6">
            <div className="bg-[#1A05A2] text-white text-center py-2.5 font-black text-base flex justify-center items-center gap-2 w-max mx-auto px-8 sm:px-10 rounded-b-xl absolute top-0 inset-x-0 shadow-sm z-10">
              <span className="text-xs">🌿</span>{" "}
              {isEn ? "Club Rules & Guidelines" : "क्लबाचे नियम व सूचना"}{" "}
              <span className="text-xs">🌿</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 mt-1">
              {rules.map((rule, idx) => (
                <div key={idx} className="flex items-start gap-2">
                  ...
                  <span className="text-xs sm:text-sm font-black text-[#1a1a40] leading-snug">
                    {rule}
                  </span>
                </div>
              ))}
          ...
          {/* Happy Moments */}
          <div className="col-span-1 lg:col-span-3 bg-white rounded-[24px] shadow-lg border-2 border-pink-200 overflow-hidden relative flex flex-col justify-between min-h-75">
            <div className="bg-[#f472b6] text-white text-center py-2.5 font-black text-base flex justify-center items-center gap-2 w-max mx-auto px-6 rounded-b-xl absolute top-0 inset-x-0 z-10 shadow-sm">
              <span className="text-xs">✦</span>{" "}
              {isEn ? "Moments of Happiness" : "आनंदी जीवनाचे सुंदर क्षण"}{" "}
              <span className="text-xs">✦</span>
            </div>
            <div className="w-full flex-1 pt-14 p-3 flex flex-col">
              <div className="w-full flex-1 min-h-55 rounded-2xl overflow-hidden border-2 border-pink-100 relative shadow-inner">
                <img
                  src="/images/aandmelav 10.jpeg"
                  alt="Happy Seniors"
